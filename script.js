...
button.addEventListener('click', function() {
        ...
        // console.log(secretWord);
        // console.log(guessesRemaining);
        guessesRemaining--; //subtracts from guesses remaining
...
function getUserInput() {
    userInputArray.length = 0; //Resets user input each guess, otherwise the user input array just gets added to & code after this gets screwed.
    for (const input of document.querySelectorAll('.currentattempt input')) { //selecting currentattempt div but document.querySelectorAll... returns nodelist array of input.tiles (e.g. 0: input#firstletter.tile ++). This loop iterates over that array. 
        userInputArray.push((input.value).toLowerCase()); //At each iteration, grabs value from each tile, and pushes that value into the userInputArray
    }
    checkIfUserInputIsAWord(); //Makes sure that the user input guess is actually a valid, 5 letter word.
}

function checkIfUserInputIsAWord() {
    if (!words.includes(userInputArray.join(''))) { //checks if userinput is a word
        trigger = false;
    }
    if (words.includes(userInputArray.join(''))) {
        trigger = true;
    }
}
    ...
    for (let i = 0; i < attempts.length; ++i) {
        if (attempts[i].classList.contains('currentattempt')) {
            attempts[i].classList.remove('currentattempt');
                for (const tiles of attempts[i].children) {
                    tiles.setAttribute('disabled', 'true');
                }
